...
int main(int argc, char **args)
{
  ...
  PetscCall(PetscOptionsGetBool(NULL, NULL, "-use_memtype", &use_memtype, NULL));
  PetscOptionsEnd();
  if (lda > 0) lda += n;
  ...
  if (use_memtype) PetscCall(MatCreateDenseFromMemType(PETSC_COMM_WORLD, A_memtype, n, n, PETSC_DECIDE, PETSC_DECIDE, lda, array, &B));
